/**
 * Utility for parsing RGB/RGBA color strings and extracting RGB components
 * @updatedAt 2025-08-16
 */

/**
 * Parses RGB or RGBA color string and returns normalized RGB values
 * @param colorString - Color string in format "rgb(r, g, b)" or "rgba(r, g, b, a)"
 * @returns Array of RGB values normalized to 0-1 range [r, g, b] or null if invalid
 */
export function parseRgbColor(colorString: string): number[] | null {
  ...
  // Return normalized RGB values (0-1 range)
 ...
 * @returns Array of normalized RGB color arrays or null if invalid
 */
export function parseColorArray(colors: string[]): number[][] | null {
  ...
  for (const colorString of colors) {
    const parsedColor = parseRgbColor(colorString);
    if (!parsedColor) {
